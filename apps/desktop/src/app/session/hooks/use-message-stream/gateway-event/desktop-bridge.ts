...
export function handleDesktopBridgeEvent(ctx: GatewayEventContext): boolean {
  ...
  // same-id background source from driving the visible surface.
  const ownsActiveSurface = Boolean(
    ctx.sessionId && ctx.sessionId === ctx.deps.activeSessionIdRef.current && ctx.fromActiveSource()
  )
