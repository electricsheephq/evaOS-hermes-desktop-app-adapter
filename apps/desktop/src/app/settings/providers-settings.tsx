...
function OAuthPicker({
  ...
  const featured = managedEva ? null : (ordered.find(p => p.id === FEATURED_ID && !isConnected(p)) ?? null)
...
export function ProvidersSettings({
  ...
  if (showApiKeys) {
    ...
    return (
      <SettingsContent>
        {!managedEva && <SettingsProfileScope className="mb-5" />}
        {!managedEva && <LocalEndpointRow onOpen={reason => startManualLocalEndpoint(reason, scopeProfile)} />}
        {keyGroups.length > 0 ? (
