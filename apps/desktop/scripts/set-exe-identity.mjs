#!/usr/bin/env node
// set-exe-identity.mjs — stamp the evaOS Agent icon + version metadata onto the
...
//   node scripts/set-exe-identity.mjs <path-to-evaOS-Agent.exe>
//
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..'), editExecutable = rcedit) {
  ...
  const icon = join(desktopRoot, 'assets', 'eva.ico')
  if (!existsSync(icon)) {
  ...
  console.log('[set-exe-identity] done — evaOS Agent icon + identity stamped')
}
