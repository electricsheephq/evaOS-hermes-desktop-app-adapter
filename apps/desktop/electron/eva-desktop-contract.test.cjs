...
test('evaOS Agent package identity and customer artifact contract are exact', () => {
  ...
  assert.equal(pkg.version, '2026.7.20-es.1')
...
test('managed session persistence encrypts both tokens and restricts the state file', () => {
  ...
  assert.match(runtime, /assertEvaManagedApiRequestAllowed\(request, \{ agentId: runtime\.agentId \}\)/)
  assert.match(runtime, /assertEvaManagedApiRequestAllowed\(request, \{ agentId: refreshed\.agentId \}\)/)
...
test('managed member UI exposes one server-bound agent context without profile or integration controls', () => {
  ...
  assert.match(sidebar, /!showSessionSections && !managedEva/)
  assert.match(sidebar, /!managedEva && \([\s\S]*?<ProfileRail \/>/)
...
test('updater and editable gateway paths fail closed in the managed build', () => {
  ...
  assert.match(main, /EVA_MANAGED_BUILD \? managedUpdateResponse\('check'\)/)
  assert.match(main, /EVA_MANAGED_BUILD \? managedUpdateResponse\('apply'\)/)
