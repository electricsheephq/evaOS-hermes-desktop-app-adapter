...
class TestManagedExternalGatewayRestart:
    ...
    def test_managed_runtime_hands_off_to_exact_external_supervisor(
        ...
        monkeypatch.setattr(
            status, "get_running_pid_identity_strict", lambda _pid_path: (4242, 12.5)
        )
        monkeypatch.setattr(
            gateway_cli,
    ...
    def test_managed_runtime_refuses_unverified_supervisor(self, monkeypatch):
        ...
        monkeypatch.setattr(
            gateway_cli, "_capture_gateway_argv", lambda pid: ["hermes", "gateway", "run"]
        )
        monkeypatch.setattr(
            gateway_cli,
            "_graceful_restart_via_sigusr1",
    ...
    def test_replacement_wait_requires_running_external_supervisor(
        ...
        runtime_path.write_text(
            json.dumps({"pid": 5252, "start_time": 22.5, "gateway_state": "starting"})
        )
        monkeypatch.setenv("HERMES_HOME", str(home))
        monkeypatch.setattr(gateway_cli, "get_hermes_home", lambda: home)
        monkeypatch.setattr(
            status, "get_running_pid_identity_strict", lambda _pid_path: (5252, 22.5)
        )
        monkeypatch.setattr(
            gateway_cli,
            "_capture_gateway_argv",
    ...
    def test_replacement_wait_rejects_matching_startup_failure(
        ...
        (home / "gateway_state.json").write_text(
            json.dumps(
                {"pid": 5252, "start_time": 22.5, "gateway_state": "startup_failed"}
            )
        )
        monkeypatch.setenv("HERMES_HOME", str(home))
        monkeypatch.setattr(gateway_cli, "get_hermes_home", lambda: home)
