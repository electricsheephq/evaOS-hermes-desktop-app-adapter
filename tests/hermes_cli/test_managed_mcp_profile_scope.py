...
def test_profile_with_only_managed_mcp_starts_discovery(tmp_path, monkeypatch):
    from hermes_cli import mcp_startup

    _, _, louis = _setup_scopes(tmp_path, monkeypatch)
    (louis / "config.yaml").write_text("display:\n  skin: louis\n", encoding="utf-8")
    managed = tmp_path / "managed" / "louis"
    _write_yaml(
        managed / "config.yaml",
        {"mcp_servers": {"managed-only": {"url": "https://managed.example/mcp"}}},
    )
    _clear_config_caches()
...
def test_multiplex_sessions_discover_managed_mcp_per_profile(
    ...
    def _fake_discover():
        ...
        for server_name in mcp_tool._load_mcp_config():
            ...
            with mcp_tool._lock:
                mcp_tool._servers[server_name] = SimpleNamespace(
                    session=object(),
                    _registered_tool_names=[tool_name],
                    _sampling=None,
                )
            added_state_keys.append((home, server_name))
    ...
    set_multiplex_active(True)
    added_state_keys.append((None, base_state_key))
    ...
    try:
        ...
        for home, state_key in added_state_keys:
            if home is None:
                set_multiplex_active(False)
