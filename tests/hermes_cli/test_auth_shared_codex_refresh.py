...
def _shared_refresh_worker(
    ...
    def _refresh(*_args, **_kwargs):
        with calls.get_lock():
            calls.value += 1
        # Keep the first writer in the critical section long enough for the
        # sibling to contend on the managed source lock.
        time.sleep(0.25)
        return {
...
def test_shared_refresh_writes_provider_and_matching_pool_without_profile_shadow(
    ...
    old_access = _synthetic("old-access")
    old_refresh = _synthetic("old-refresh")
    new_access = _synthetic("new-access")
    new_refresh = _synthetic("new-refresh")
    _write_json(
        managed_profile_env["shared"],
        _auth_store(
...
def test_shared_cli_recovery_writes_back_to_shared_source_without_profile_shadow(
    ...
    recovered_access = _synthetic("recovered-access")
    recovered_refresh = _synthetic("recovered-refresh")
    _write_json(
        managed_profile_env["shared"],
        _auth_store(
            access_token=None,
...
def test_invalid_shared_refresh_recovers_to_shared_source_without_profile_shadow(
    ...
    old_access = _synthetic("invalid-old-access")
    old_refresh = _synthetic("invalid-old-refresh")
    recovered_access = _synthetic("invalid-recovered-access")
    recovered_refresh = _synthetic("invalid-recovered-refresh")
    _write_json(
        managed_profile_env["shared"],
        _auth_store(
            access_token=old_access,
...
def test_shared_pool_fallback_is_used_without_creating_local_shadow(
    ...
    pool_access = _synthetic("pool-only-access")
    pool_refresh = _synthetic("pool-only-refresh")
    _write_json(
        managed_profile_env["shared"],
        _auth_store(
            access_token=None,
...
def test_local_codex_refresh_remains_profile_local_without_managed_source(
    ...
    old_access = _synthetic("local-old-access")
    old_refresh = _synthetic("local-old-refresh")
    new_access = _synthetic("local-new-access")
    new_refresh = _synthetic("local-new-refresh")
    _write_json(
        managed_profile_env["profile"] / "auth.json",
        _auth_store(
            access_token=old_access,
            refresh_token=old_refresh,
        ),
...
def test_force_refresh_ignores_unrelated_future_last_refresh(
    ...
    old_access = _synthetic("future-old-access")
    old_refresh = _synthetic("future-old-refresh")
    new_access = _synthetic("future-new-access")
    new_refresh = _synthetic("future-new-refresh")
...
def test_shared_refresh_is_serialized_and_second_profile_adopts_rotated_pair(
    ...
    processes = [
        context.Process(
            target=_shared_refresh_worker,
            args=(
                str(profile),
                str(managed_profile_env["shared"]),
                calls,
                start_event,
                initial_read_barrier,
                initial_reads,
