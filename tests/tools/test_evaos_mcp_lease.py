...
@pytest.mark.asyncio
async def test_real_sdk_http_client_mounts_lease_and_refreshes_once_after_401(
    tmp_path,
):
    """MCP2's real httpx2 client accepts the SDK-selected Auth adapter."""
    httpx2 = pytest.importorskip("httpx2")
    from tools.mcp_tool import sdk_httpx

    sdk = sdk_httpx()
    assert sdk is httpx2
    ...
    def handler(request):
        ...
        return sdk.Response(status, request=request)

    async with sdk.AsyncClient(
        transport=sdk.MockTransport(handler),
        auth=EvaosLeaseHttpAuth(manager),
    ) as client:
    ...
    for _url, headers in requests:
        assert headers["x-pd-project-id"] == "proj"
        assert headers["x-pd-environment"] == "production"
        assert headers["x-pd-external-user-id"] == "server-derived"
        assert headers["x-pd-app-slug"] == "google_sheets"
        assert headers["x-pd-account-id"] == "apn_test_server_resolved"
