...
class TestTurnRunner:
    ...
    def test_profile_mcp_discovery_happens_before_fresh_agent_snapshot(
        self, monkeypatch
    ):
        events = []
        ...
        ctx = TurnContext(
            ...
            session_key="agent:eve:local:dm:profile-chat",
            user_config={},
            AIAgent=_SnapshotAgent,
            resolve_display_setting=lambda *_args: False,
            _run_still_current=lambda: True,
            _hooks_ref=SimpleNamespace(loaded_hooks=False),
        )
        ...
        monkeypatch.setattr(
            "gateway.run._prepare_mcp_registry_for_gateway_agent",
            lambda: events.append("profile-discovery"),
        )
        ...
        assert events[:2] == ["profile-discovery", "agent-snapshot"]
