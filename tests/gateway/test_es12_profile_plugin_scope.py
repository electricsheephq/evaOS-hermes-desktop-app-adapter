...
def test_profile_discovery_can_run_concurrently_across_scoped_managers(
    two_profiles, monkeypatch
):
    """Current upstream isolates managers without a process-wide discovery lock."""
    from gateway.run import _profile_runtime_scope
...
def test_profiles_isolate_an_identical_directory_plugin_module(tmp_path):
    ...
    assert first.module.__name__.startswith("hermes_plugins.shared_relative")
    assert second.module.__name__.startswith("hermes_plugins.shared_relative")
...
def test_profiles_apply_import_time_tool_override_policy_independently(tmp_path):
    ...
    try:
        ...
        assert all(
            name.startswith("hermes_plugins.scoped_override")
