...
## Current PR review dispositions

- Shared priority mutation: `MERGE_BLOCKING / FIXED NOW`. Upstream adds `move_entry`; managed fallback must not reorder administrator-owned credentials. Added the same profile-shadow ownership guard used for removal. `tests/hermes_cli/test_auth_profile_fallback.py::test_managed_profile_cannot_reorder_shared_credentials` RED (no PermissionError) then GREEN; full file 21 passed. Shared file remains byte-identical and a profile-owned override remains editable.
- Session-control generation reuse: `MERGE_BLOCKING / FIXED NOW`. Clearing the generation map allowed an old read to reuse the first post-switch token for the same session. Retain monotonic generations, preserving the r31 callback/routing/reconnect invariant. New returning-session case in `apps/desktop/src/store/session-control.test.ts` RED (stale snapshot published), then full file 25 passed.
- Trusted `_blank` links: `RELEASE_BLOCKING / ESCALATED` to the orchestrator's Desktop es.6 pairing under cadence rule 2, outside this PR-only runtime-source cut. Three inherited upstream anchors (onboarding DocsLink, messaging provider docs, statusbar links) lack the existing IPC link helper and are denied by upstream's unconditional window-open security policy. Preserve denial; route the trusted anchors through the existing helper before Desktop release. Duplicate bot reports are one finding; PR review threads retain the exact paths.
- Obsolete `max_tokens` recovery notices: `NON_BLOCKING / ACCEPTED TRADEOFF` for this cut. Overflow/truncation notice modules are byte-identical to base and upstream; their setting advice conflicts with new docs. No changed managed recovery failure was established. Automatic provider-budget recovery still has an ephemeral cap path.
- CodeQL: failed advisory scan is retained, not silently called green. Its large-diff report counted 917 alerts; all five critical IDs in the PR's current alert set also occur on frozen main with unchanged sink logic. Of 57 PR-only IDs, bounded AST comparison matches 25 functions/sink statements to frozen base; 19 unmatched reports are under `evals/`. Remaining signals are not a security-clearance claim. No alert was dismissed or scan policy weakened. Exact-head named workflow results and the unresolved scanner boundary belong in the final PR/cut receipt.
