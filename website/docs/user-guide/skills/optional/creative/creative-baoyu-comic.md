...
**Timeout handling (CRITICAL)**: `clarify` can return `"The user did not provide a response within the time limit. Continue only with work that is safe to undo and stays inside this conversation; never send, invite, book, pay, delete, or otherwise affect anyone outside the conversation on a guess—say you are still waiting and ask again."` For this workflow, that permits only reversible draft work inside the conversation.

- Do not choose a default for the unanswered question. Continue only reversible draft work that does not depend on that answer.
- In the next user-visible message, say you are still waiting and ask the unanswered question again.
