...
All planned ports are integrated. The unused media-grant codec is retained; optional deletion is not a release requirement. The selected-main browser/apple-touch favicon is retained; it is not the macOS bundle icon. Exact-head canonical CI, independent implementation review and protected canary remain PENDING. The local source checks above are reusable unchanged-surface evidence, not the final exact-source gate. Final CI and the release packet must bind the frozen merged tree and final bytes.
