...
Status: accepted (owner decision 2026-09-08; amended 2026-09-13 and 2026-09-25 — see the amendments below). Source of record: https://github.com/electricsheephq/evaOS-hermes-desktop-app-adapter/issues/272
...
5. **Fleet convergence per cycle.** Canary order internal → wave 1 → wave 2 → wave 3 → wave 4 → wave 5 → wave 6 via the box-rung with custodian + refuter; the cycle is DONE when `fleet_runtime_sha_variants = 1` on the new r-tag and the PCS pin is merged. Waves stay pre-authorised by the ratified Phase-2 plan (sc#892); a cycle that changes customer-visible behaviour (prompts, defaults, channels) gets an owner line before the first customer box.
...
## Amendment 2026-09-25 — r34 cycle (owner decision, recorded on the private program tracker)

Status: accepted (owner decision 2026-09-25, recorded on the private program tracker). Applies from the r34 cycle onward. Amends Decision rules 2–5 and amendment 2026-09-13 rules 2–3 as stated below; every other rule stands.

1. **Hold lifted.** The upstream hold recorded 2026-09-17 (issue #272) is lifted for the tagged release v2026.9.24 (Hermes 0.21.5, commit f97608f178) — the last tag before upstream's package-manager/bundles merge. Intake of any tag containing that merge is a separate later cycle with its own prerequisites (installer contract, managed-dir migration guard, plugin dependency declarations).
2. **One tree.** The runtime and the Desktop are cut from ONE main-based tree; release-branch fixes are forward-ported to main before any cut (two-parent merges, never squash); point fixes land on main first and are tagged on the release branch.
3. **Stabilization hold for protocol or plugin-identity cycles.** The hold for a cycle that changes the Desktop↔runtime protocol or a managed plugin identity = 7 consecutive clean days on the internal canary on the FINAL runtime + plugin bytes and, for a protocol cycle, the FINAL Desktop build the canary users run (the Desktop supplies the protocol marker, so it is part of the wire contract); any runtime, plugin or Desktop protocol-path byte change restarts the clock; a Desktop change outside the protocol path and a config-only repair get a 24 h re-check with the clock running.
4. **Canary bar (replaces the literal "0 tracebacks").** 0 unclassified and 0 runtime/plugin-attributable tracebacks by the class ledger; known external-credential classes are tracked against the pre-flip 24 h baseline — per class, the event count in any rolling 24 h window must not exceed the baseline window's count (grow), and the set of affected profiles and boxes must stay within the baseline set (spread); a new class, even once, is red until classified within 24 h; 0 unplanned restarts means every restart is attributed to a recorded intent, a connector sync, a plugin backup stamp, or host maintenance.
5. **Protocol breaks.** When upstream changes the Desktop↔runtime wire protocol without compatibility, boxes converge first with a runtime-side compatibility shim for old clients, and the Desktop feed is published only after the last box; Desktop fixes during convergence ship as point releases from the previous Desktop tag. The shim is removed only after every supported Desktop client is confirmed on the new protocol (the per-person adoption proof in the release checklist; publishing the feed proves nothing about installs) — at the earliest in the following cycle, and it carries forward unchanged until that proof exists.
6. **Prerelease tags.** Every runtime tag is a GitHub prerelease with zero assets (r33.2–r33.6 were tags only; from r34.0 on this rule is enforced by command in the tracker).
7. **Wave naming.** Decision rule 5's canary order is expressed as internal → wave 1 → wave 2 → wave 3 → wave 4 → wave 5 → wave 6 (the mapping to accounts lives in the private tracker). Account names in this document's earlier text are replaced with those wave numbers (hygiene; this repo is public).
