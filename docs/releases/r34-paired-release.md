...
| LCM-X plugin | v0.24.1 / `98ac62fee5316cdf461e77f28951720f523489f1` (electricsheephq/lcm-x, tag object `ff6035202e405e3d6e613c416890a1b0eb191633`); plugin `hermes-lcm-x`, context engine `lcm-x`, replaces plugin name `hermes-lcm`; native recovery OFF on every qualified host (v0.24.1 release notes; eva and the rs34 cell run OFF) |
...
| PCS | 0.1.143 (managed payload builder, #957; LCM-X migrate/rollback modes, #959), 0.1.145 (the #959 label after review), 0.1.146 (#961 follow-ups). These carry the v0.24.0 candidate pin (`f3e56f9c…`, source `d346ab9e…`). The v0.24.1 pin plus `LCM_NATIVE_RECOVERY` as a migrate/rollback parameter with fleet default `false` is PCS 0.1.147 (draft; must merge before the first box; the label may renumber if the stability lane's 0.1.148 merges first) |
...
- LCM-X: the pinned source loads as `hermes-lcm-x` / `lcm-x` from a config that names only the new identity (`tests/r34_compat/test_pinned_lcmx_plugin.py`). The plugin rename is a config change PCS performs (`plugins.enabled`, `context.engine`); a box that enables only `hermes-lcm` falls back to the built-in compressor (same file, through the real plugin loader and engine selection).
...
Per box, on a red bar only, the operator kit `RERUNG-r34` (support-control, merged in sc#1016) runs the rungs in this order, and never skips a rung. Runtime: stop both units; restore `state.db` from the pre-flip backup the driver takes before each rung (r34 moves FTS storage v2 → v3 on first open); re-rung on the predecessor runtime and read back. LCM-X, only if the profile was migrated, as a ladder: **L1** first — PCS `rollout-lcm-x-profile.sh rollback --to manifests/lcm-x-0.24.0.v1.json` re-pins `hermes-lcm-x` to 0.24.0 (measured clean: 0 re-stored rows, the next compaction commits); **L2** — `rollback` to the frozen `manifests/lcm-x-0.23.3.v1.json` (shipped by PCS 0.1.147; a draft until that PR merges — no L2 before it), which re-enables `hermes-lcm` 0.23.3 and unlinks `hermes-lcm-x`, is taken only when L1 failed or was insufficient AND the owner approved it for that box (L2 is an owner gate; the measured effect on the same `lcm.db` is no loss, the rows above the last frontier re-store once, and conversations that compacted under 0.24.1 stop compacting until they rotate — start fresh sessions after an L2). Fleet posture throughout: `LCM_NATIVE_RECOVERY=false`. Every LCM-X rung is configuration plus plugin payload only: the live `lcm.db` stays in place, and its pre-migrate copy is restored only if the target plugin cannot open it. A failed canary holds the cycle: no feed flip, no PCS pin, predecessor restored (Decision rule 6). `tests/r34_compat/test_state_rollback_compat.py` shows r33.6 still reads, appends and searches a state.db that r34 opened or created; the kit does not rely on it.
...
- LCM-X store continuity across a runtime or plugin rollback is not in CI; only the v0.24.1 plugin's own restart on one store is. Measured outside CI (lcm-x#471, comment 5841246018): v0.24.1 → v0.23.3 on the same `lcm.db` loses nothing; rows above the last frontier re-store once (11 in the measurement); a conversation that compacted under v0.24.1 cannot compact again on v0.23.3 until it rotates, so an L2 rollback means fresh sessions. L2 stays an owner gate.
