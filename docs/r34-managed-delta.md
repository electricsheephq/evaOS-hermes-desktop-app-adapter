...
| es.9 connector replies (PR-2b) | `rpc_dispatch._handle_admitted_request`, after `check_result` | Only for a request that arrived in the es.9 shape: `connectors.list` rows also carry camelCase `connectionStatus`; `connectors.connect` answers r33's `{results: [{connector, status: active if connected else initiated, connect_url}], summary: {}}` from the operation view, and from the open operation (`live.current`, the transport's own session) where the runtime says `4002 LINK_STILL_VALID`. `4004` and every other error stay errors (no r33-style minting). The native contract is not widened. |
...
- es.9 `connectors.list` / `connectors.connect` send `{session_id}`; the tag requires `owner`. PR-2 (§SHIM) carries the params over. The reply gap is closed by PR-2b (`tui_gateway/legacy_prompt_shim.py:165` `_legacy_connector_reply`, hooked at `tui_gateway/rpc_dispatch.py:53`): es.9-shaped list rows also carry `connectionStatus`, and connect answers `{results, summary}` on the operation view and on `4002 LINK_STILL_VALID`; tests `test_pr2b_*` in `tests/tui_gateway/test_legacy_prompt_shim.py`. Still open: `4004 UNKNOWN_OPERATION` (no connection operation open) stays an error for es.9, which r33 answered by minting; es.9 can authorize a connector only while an agent `manage_connections` call holds an operation open; a connect with no open operation still errors.
