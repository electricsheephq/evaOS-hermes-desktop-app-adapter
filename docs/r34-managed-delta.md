...
| R3 | MCP scope / lease / live state per the re-port map (fleet unit + desktop unit) | RED: tag without fork, 8 files / 46 tests fail. GREEN: 33 files / 276 tests, exit 0. New: a live `evaos_lease` connection for another account was adopted at the tag (account not in the connection identity); fixed by RE-6 (RED without it, GREEN with it). Residue sweep with positive controls: 0 runtime hits Review round (RE-7): at `23e17286a7`, `connection.request` was emitted to every viewer with no protocol check. Now, on a Desktop session, `_desktop_ui_emit_to_requester` delivers it only to the viewer whose prompt claimed the running turn. That viewer is recorded as `_turn_requester_transport` at prompt submit and at a queued-prompt drain, and is cleared on goal, auto-continue and notification turns. The frame goes to that peer's own mailbox (`FanoutTransport.write_to`), and only when that viewer's own entry in `viewers` is `desktop` with protocol ≥ 2 (`"connection.request": (2, "manage_connections")`). Every other viewer gets nothing, and other session traffic still fans out. With no resolvable requester, the card is refused. The TUI keeps the session emit. `tests/tui_gateway/test_connection_request_guard.py` uses the production `FanoutTransport` with two viewers (requester on protocol 2; the other on 1, or on 2). RED at `b56c1a32c1` (4 of 7: the other viewer received the card; no-requester and protocol-1-requester cases also got it) / GREEN (7 passed). The first-round tests at `23e17286a7` proved only the protocol refusal. | `r3/RED-tag-without-fork.txt`, `r3/GREEN-final-head.txt`, `r3/N3-RED-without-RE6.txt`, `r3/residue-sweep.txt`, `review-round/RE7-connection-guard-RED-23e1728.txt`, `review-round/RE7r2-isolation-RED-b56c1a3.txt`, `review-round/RE7r2-isolation-GREEN.txt` |
