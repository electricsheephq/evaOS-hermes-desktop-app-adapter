...
| R3 | MCP scope / lease / live state per the re-port map (fleet unit + desktop unit) | RED: tag without fork, 8 files / 46 tests fail. GREEN: 33 files / 276 tests, exit 0. New: a live `evaos_lease` connection for another account was adopted at the tag (account not in the connection identity); fixed by RE-6 (RED without it, GREEN with it). Residue sweep with positive controls: 0 runtime hits Review round (RE-7): `connection.request` was emitted to every viewer with no protocol check. It now goes through `_desktop_ui_emit` at protocol 2 (`"connection.request": (2, "manage_connections")`), only on the requesting viewer's transport; the TUI keeps the session emit. RED at `23e17286a7` (2 of 3 tests; the TUI control passes) / GREEN (3 passed). | `r3/RED-tag-without-fork.txt`, `r3/GREEN-final-head.txt`, `r3/N3-RED-without-RE6.txt`, `r3/residue-sweep.txt`, `review-round/RE7-connection-guard-RED-23e1728.txt`, `review-round/RE7-connection-guard-GREEN.txt` |
...
- RE-7 from the re-port map was first recorded here as not carried. That was wrong. `setup_mcp` is COVERED-UPSTREAM, but its replacement card `connection.request` still needed the protocol and requesting-viewer gate. The review round carried it (R3 row). On a protocol-1 Desktop the card is not sent; the tool then waits for its 300s operation deadline, like a card sent to a client that cannot render it. Test adaptations: `test_connector_operation_e2e`'s fixture declares protocol 2; `test_r31_ui_protocol_contract` expects the new table entry.
