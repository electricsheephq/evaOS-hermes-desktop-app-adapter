...
| Legacy answers | `rpc_dispatch._handle_admitted_request` | `<kind>.respond` delegates to upstream `request.answer` (batch per-question: `clarify.lock`) only when the open request's srq method is the mapped kind (PR-2b; compute-host mirror included), else the legacy 4009 and nothing settles. `clarify.respond` is exempt from that check and behaves exactly as before PR-2b (per-question `clarify.lock`; cancel-all `request.answer`). The native `request.answer` stays id-keyed as upstream and is unchanged. First answer wins; a late one is `expired`; no `request_id` is the legacy `4009 no pending <key> request`. Never registered, never shadows a real handler, not in the contract catalog. |
...
| es.9 connector replies (PR-2b) | `rpc_dispatch._handle_admitted_request`, after `check_result` | Only for a request that arrived in the es.9 shape: `connectors.list` rows also carry camelCase `connectionStatus`; `connectors.connect` answers r33's `{results: [{connector, status: active if connected else initiated, connect_url}], summary: {}}` from the operation view, and, where the runtime says `4002 LINK_STILL_VALID`, from the open operation's targets sampled BEFORE dispatch (`_legacy_connector_snapshot`, the transport's own session); nothing is re-read after dispatch, so an operation that settles or is succeeded meanwhile is never sampled. `4004` and every other error stay errors (no r33-style minting). The native contract is not widened. |
| Legacy mark on create (PR-2b) | `methods_session._create_session` | A never-advertised WS client creating a session marks it legacy, so a prompt raised while es.9 is detached before its first prompt still twins into the replay ring. |

Covered kinds: clarify (single, batch, cancel-all), approval (twin only; `approval.respond` / `.pending` / `.received` are native, no `.expire`), sudo, secret, tour, and the three vault prompts (`vault.unlock_prompt` as `vault.unlock.*`, `vault.save_login`, `vault.code`). The vault kinds are adopted by orchestrator decision: they block the user, and the shim spec recommended them. Not covered, so still failing fast for a legacy-only session: the GUI reads `terminal.read` / `preview.read` / `preview.act` / `window.read` (the tools report no live surface) and `connection.request` (see KNOWN GAPS). Upstream tests edited: two `test_protocol` cases that assert the fail-fast now pin the shim off; `contracts/test_es9_wire_compat` runs the es.9 connector shapes through `_normalize_request` (was a strict xfail).

Known edges (NON_BLOCKING for PR-2):

- Detached before the first prompt (review mL-SK): closed by PR-2b. `_lp_seen` is now also set when an unadvertised WS transport runs `session.create` (`tui_gateway/methods_session.py:398` → `legacy_prompt_shim._legacy_prompt_mark`, `tui_gateway/legacy_prompt_shim.py:132`); test `test_pr2b_es9_create_marks_the_session_so_a_detached_sudo_is_replayed`.
- Mixed es.9 + es.10 session (review mL-SX): the first answer settles the request, but the losing client's card stays until its own answer returns `expired` (legacy) or is dropped (new); no withdrawal is sent on settlement. Follow-up: adapter#356.
- Connector 4002 sample window (PR-2b round 2, ACCEPTED TRADEOFF): the es.9 `results` for a `LINK_STILL_VALID` reply are built from the operation sample taken before dispatch (`_legacy_connector_snapshot`). If operation A is replaced by a successor B between that sample and the handler's own read, and B answers 4002, the reply carries A's link once; es.9 opens it, gets a dead link, and the user retries (authority stays confined to the transport's own session; no data changes). The 4002 payload does not identify its operation, so closing the window needs a native contract change — out of the shim's scope; retires with the shim.
- Legacy clarify cancel-all on another open kind (PR-2b round 2, pre-existing, FOLLOW-UP adapter#356): `clarify.respond` is exempt from the cross-kind check by decision, so it behaves exactly as base (PR #369): a cancel-all whose `request_id` belongs to an open sudo settles that sudo with an empty answer (the sudo tool's skip/fail-gracefully path, `tools/terminal_tool_sudo.py:185` — a denial, not an authorization); a per-question answer returns `expired` and leaves the sudo open. es.9 only sends a cancel-all for a request id it received as a clarify, so the edge needs a misbehaving client; pinned by `test_pr2b_legacy_clarify_respond_on_another_open_kind_is_unchanged_from_base`.

Kill switch rationale (review mL-SS): an operator emergency lever set in the managed unit environment, flipped by provisioning without touching the pinned per-profile `config.yaml`; it is not a user setting and retires with es.9 (so it is not added to config). Flip recipe: the value is read per call, but a unit-env change needs a gateway restart; a live flip needs the key in `~/.hermes/.env` plus the `reload.env` RPC. Flipping it off mid-wait orphans the open es.9 card until the request's timeout: its `*.respond` then answers `-32601` and no `*.expire` is sent.
...
- es.9 `connectors.list` / `connectors.connect` send `{session_id}`; the tag requires `owner`. PR-2 (§SHIM) carries the params over. The reply gap is closed by PR-2b (`tui_gateway/legacy_prompt_shim.py:180` `_legacy_connector_reply`, hooked at `tui_gateway/rpc_dispatch.py:54`; the 4002 sample `tui_gateway/legacy_prompt_shim.py:166` `_legacy_connector_snapshot`, hooked at `tui_gateway/rpc_dispatch.py:25`): es.9-shaped list rows also carry `connectionStatus`, and connect answers `{results, summary}` on the operation view and on `4002 LINK_STILL_VALID`; tests `test_pr2b_*` in `tests/tui_gateway/test_legacy_prompt_shim.py`. Still open: `4004 UNKNOWN_OPERATION` (no connection operation open) stays an error for es.9, which r33 answered by minting; es.9 can authorize a connector only while an agent `manage_connections` call holds an operation open; a connect with no open operation still errors.
...
| `test_pinned_lcmx_plugin.py` | the pinned source loads as plugin `hermes-lcm-x` and registers context engine `lcm-x` from a config naming only the new identity; engine ingest, `lcm_grep` and restart on one `lcm.db`; a profile that enables only `hermes-lcm` (engine `lcm` or `lcm-x`) loads cleanly, registers no engine, and the agent's engine selection falls back to the built-in compressor | the load test fails against the r31 source (`--r34-lcmx-source`/`--r34-lcmx-ref`) |
| `test_es9_session_bind_wire.py` | `desktop_ui_protocol` and its method set come from the pinned data contract `tests/tui_gateway/contracts/es9_protocol.json` (recorded once from the es.9 tag and r33.6, which agree; the test reads no TypeScript); every request gets a unique id and only frames after its dispatch are scanned; every recorded es.9 create (5), resume (7) and activate (1) shape binds, a clarify raised mid-turn is answered with the legacy `clarify.respond`, and every rebind carries `pending_clarify` | `HERMES_EVAOS_LEGACY_PROMPT_SHIM=0`: binds still succeed, but no twin, no `pending_clarify`, `-32601`, clarify timeout; the raw tag additionally refuses the bind key itself (4000, receipt) |
