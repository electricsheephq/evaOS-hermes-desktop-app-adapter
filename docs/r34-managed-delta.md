...
- Detached before the first prompt (review mL-SK): `_lp_seen` is set only when a twin is wanted or a snapshot is read, not on `session.create`. A covered prompt raised while es.9 is detached and before `_lp_seen` is set writes only the `srq` frame, so sudo, secret, tour and vault prompts never enter the replay ring and time out; clarify is restored via `pending_clarify` on the next resume/activate. Fix (set `_lp_seen` when an unadvertised WS transport runs `session.create`) is deferred to PR-2b.
...
- es.9 `connectors.list` / `connectors.connect` send `{session_id}`; the tag requires `owner`. PR-2 (§SHIM) carries the params over; the replies are the r34 ones and do not match what es.9 reads: `connectors.list` rows use snake_case `connection_status` while es.9 reads `connectionStatus` (its reconnect choice), and `connectors.connect` for a session answers from an open connection operation (`4004 UNKNOWN_OPERATION` when there is none, else the operation view) with no `results`, which es.9's connector card needs. Both go to PR-2b before the customer wave: PR-2b is RELEASE_BLOCKING for customers, not for PR-2.
